...
app = Quart(__name__)
app.secret_key = "VIAH_BASS_V4_MASTER_FINAL"

# --- FIREBASE SECURE SYNC ---
fb_config = os.environ.get("FIREBASE_CONFIG")
ref = None  # Global initialization to prevent NameError
...
if fb_config:
    try:
        ...
        ref = db.reference('/') # ref is now properly defined
...
<body class="pb-24">
    ...
    <div class="flex items-center justify-center min-h-screen p-6">
        <div class="w-full max-w-sm glass p-10 rounded-[50px] text-center shadow-2xl">
            ...
            <p class="text-[10px] text-slate-500 uppercase font-black mb-10 tracking-widest">Master Admin</p>
        ...
        <form action="/create_api" method="post" class="glass p-8 rounded-[40px] space-y-6">
            ...
            <div class="grid grid-cols-2 gap-4">
                <select name="module" class="w-full p-4 bg-slate-900 rounded-2xl border border-slate-700 outline-none text-blue-400 font-black">
                    <option value="num">📞 Number (/num)</option><option value="aadhar">🆔 Aadhar (/aadhar)</option>
                    <option value="pan">💳 PAN Card (/pan)</option><option value="vehicle">🚗 Vehicle (/vehicle)</option>
                    <option value="ip">🌐 IP Geo (/ip)</option><option value="mail">📧 Email (/mail)</option>
                </select>
            ...
            <button type="submit" class="w-full bg-blue-600 py-5 rounded-2xl font-black uppercase text-sm tracking-widest">Deploy Premium RP Key</button>
        </form>
        <div class="space-y-4">
            ...
            <div class="glass p-6 rounded-[35px] {{ 'premium-card' if data.tier == 'premium' else 'border-l-4 border-slate-600' }} space-y-4">
                <div class="flex justify-between items-start mb-4">
                    ...
                    <div class="text-right"><p class="text-xl font-black text-white">{{data.hits}}</p><p class="text-[8px] uppercase">Hits Sync</p></div>
                </div>
                <div class="bg-black/40 p-3 rounded-xl border border-white/5">
                    <p class="text-[8px] font-black text-slate-500 uppercase mb-1">Premium RP Endpoint URL</p>
...
@app.route('/')
async def index():
    if not ref: return "Database Offline. Check Render Environment Variables."
...
async def api_fetch():
    ...
    # Locked commands based on your screenshot
    cmd = f"/{api_data['module']} {target}"
    sent = await client.send_message(GROUP_ID, cmd)

    final_resp = "Analysis complete. Data match not found in registry."
    for _ in range(12):
