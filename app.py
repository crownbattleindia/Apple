...
if fb_config:
    try:
        ...
        ref = db.reference('/') # Success define
...
<body class="pb-24">
    ...
    <div class="flex items-center justify-center min-h-screen p-6">
        <div class="w-full max-w-sm glass p-10 rounded-[50px] text-center shadow-2xl">
            <h1 class="text-4xl font-black italic text-blue-500 mb-8 tracking-tighter">VIAH BASS</h1>
            <form action="/login" method="post" class="space-y-4">
                ...
                <button type="submit" class="w-full bg-blue-600 py-5 rounded-2xl font-black uppercase text-sm">Unlock</button>
            </form>
        </div>
    </div>
    {% else %}
    ...
    <main class="p-6 space-y-8">
        <form action="/create_api" method="post" class="glass p-8 rounded-[40px] space-y-6">
            ...
            <div class="grid grid-cols-2 gap-4">
                <select name="module" class="w-full p-4 bg-slate-900 rounded-2xl border border-slate-700 outline-none text-blue-400 font-black">
                    <option value="num">📞 /num</option><option value="aadhar">🆔 /aadhar</option>
                    <option value="pan">💳 /pan</option><option value="vehicle">🚗 /vehicle</option>
                    <option value="ip">🌐 /ip</option><option value="mail">📧 /mail</option>
                </select>
        ...
        <div class="space-y-4">
            ...
            <div class="glass p-6 rounded-[35px] border-l-4 border-blue-500 space-y-4">
                <div class="flex justify-between items-center">
                    <div><p class="font-black text-white italic text-lg">{{data.user}}</p><p class="text-[9px] font-mono text-slate-500 break-all">{{key}}</p></div>
                    <div class="text-right"><p class="text-xl font-black text-white">{{data.hits}}</p><p class="text-[8px] uppercase">Hits</p></div>
                </div>
                <div class="bg-black/40 p-3 rounded-xl border border-white/5">
                    <p class="text-[8px] font-black text-slate-500 uppercase mb-1">Live Endpoint</p>
...
@app.route('/')
async def index():
    if not ref: return "Firebase not connected. Check Render Environment Variables."
...
async def api_fetch():
    ...
    cmd = f"/{api_data['module']} {target}" # Commands from 1000284923.jpg
